...
def compute_votes(edgelets, model, threshold_inlier=5):
    ...
    theta = np.arccos(np.abs(np.clip(cosine_theta, -1, 1)))
...
if __name__ == '__main__':
    ...
    warped_img = rectify_image(image_name, 4, algorithm='independent')
    
    # Convert the rectified image array to unsigned 8-bit integer
    warped_img_uint8 = (warped_img * 255).astype(np.uint8)
    
    # Save the rectified image
    io.imsave(save_name, warped_img_uint8)
